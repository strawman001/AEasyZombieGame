...
public class BackpackUI : MonoBehaviour,IPointerClickHandler
{
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        foreach (RaycastResult result in results)
        {
            if (result.gameObject.name == "ItemIcon")
            {
                ...
                menu.transform.position = new Vector3(eventData.position.x+65,eventData.position.y-65,0);
                menu.SetActive(true);
                break;
            }
        }
    ...
    public void UseOrEquip()
    {
        ...
        if (target!=null)
        {
            ...
            {
                int num = consumableItems[fullItemName] - 1;
                if (num == 0)
                {
                    consumableItems.Remove(fullItemName);
                }
                else
                {
                    consumableItems[fullItemName] = num;
                }
            ...
            {
                equipmentItems.Remove(fullItemName);
                ((EquipmentItem)item).Equip();
                ShowEquipmentItems();
            }
            else if (item is WeaponItem)
            {
                weaponItems.Remove(fullItemName);
                ((WeaponItem)item).Equip();
                ShowWeaponItems();
            }
    ...
    public void ShowDescription()
    {
        ...
        if (target!=null)
        {
            String fullItemName = target.tag;
            BaseItem item = (BaseItem)GameManager.assembly.CreateInstance(fullItemName);
            descriptionPanel.transform.position = new Vector3(targetPosition.x+70, targetPosition.y-90, 0);
            (descriptionPanel.transform.GetChild(0).gameObject.GetComponent<Text>()).text = item.description;
